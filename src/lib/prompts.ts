...
export function getRecommendationQueryPrompt(
  ...
  return `以下の日報内容と課題を詳細に分析し、**課題解決に最も効果的でクリティカルなリソース**を検索するための最適な検索クエリを5つ生成してください。
...
以下のJSON形式で、検索クエリを返してください。各クエリは課題の核心を捉え、具体的な解決策や深い洞察が得られるようなものにしてください：

{
  "queries": [
    ...
    "検索クエリ4（YouTube動画検索用 - 別の角度からのアプローチ）",
    "検索クエリ5（ウェブ記事検索用 - 具体的な事例やケーススタディ）"
  ],
...
export function getRecommendationEvaluationPrompt(
  ...
  return `以下の日報内容と課題を詳細に分析し、提示されたリソース候補から**課題解決に最もクリティカルで効果的な5つ**を厳選してください。
...
以下のJSON形式で、選んだ5つのリソースを返してください。ソースのバランスを考慮し、YouTube、記事、書籍からバランス良く選んでください：
